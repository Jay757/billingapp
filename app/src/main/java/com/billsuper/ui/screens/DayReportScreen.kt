      ...
      Column(
        ...
        if (rows.isEmpty()) {
          Box(modifier = Modifier.fillMaxWidth().padding(AppSpacing.xl), contentAlignment = Alignment.Center) {
            Text("No data for selected period.", color = BillSuperColors.TextSecondary)
          }
        } else {
          LazyColumn(
  ...
  DarkCard(modifier = Modifier.fillMaxWidth()) {
    Column(modifier = Modifier.padding(AppSpacing.lg), verticalArrangement = Arrangement.spacedBy(AppSpacing.md)) {
      Row(
        ...
        Box(
          modifier = Modifier
            .background(BillSuperColors.PrimaryBlue.copy(alpha = 0.1f), RoundedCornerShape(8.dp))
      ...
      Row(
        ...
        Column(verticalArrangement = Arrangement.spacedBy(4.dp)) {
          Text("Cash: ₹ ${String.format("%.0f", row.cashTotal)}", color = BillSuperColors.TextSecondary, style = AppTypography.bodyMedium)
          Text("Online: ₹ ${String.format("%.0f", row.onlineTotal)}", color = BillSuperColors.TextSecondary, style = AppTypography.bodyMedium)
        }
        Column(horizontalAlignment = Alignment.End) {
          Text("TOTAL AMOUNT", color = BillSuperColors.TextSecondary, style = AppTypography.labelCaps)
          Text(
