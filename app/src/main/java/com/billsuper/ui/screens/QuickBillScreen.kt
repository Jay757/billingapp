...
import com.billsuper.ui.components.CircularKey
...
import com.billsuper.ui.components.BillingTotalCard
import com.billsuper.ui.components.BillSuperTable
          ...
          Row(
            ...
            Row(modifier = Modifier.weight(1.0f), horizontalArrangement = Arrangement.End, verticalAlignment = Alignment.CenterVertically) {
              ...
              IconButton(onClick = { vm.removeItem(idx) }, modifier = Modifier.size(28.dp)) {
                Icon(Icons.Outlined.Delete, contentDescription = "Delete", tint = BillSuperColors.Red.copy(alpha = 0.8f), modifier = Modifier.size(16.dp))
              }
            }
          }
      ...
      Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        DarkCard(
          ...
          Box(
            modifier = Modifier
              .fillMaxWidth()
              .background(if (mode == QuickInputMode.QTY) BillSuperColors.PrimaryBlue.copy(alpha = 0.1f) else Color.Transparent)
              .padding(16.dp),
            contentAlignment = Alignment.Center
          ) {
            Text(
              text = if (qtyText.isBlank()) "Quantity" else qtyText,
              color = if (mode == QuickInputMode.QTY) BillSuperColors.PrimaryBlue else BillSuperColors.TextSecondary,
        ...
        DarkCard(
          ...
          Box(
            modifier = Modifier
              .fillMaxWidth()
              .background(if (mode == QuickInputMode.RATE) BillSuperColors.PrimaryBlue.copy(alpha = 0.1f) else Color.Transparent)
              .padding(16.dp),
            contentAlignment = Alignment.Center
          ) {
            Text(
              text = if (rateText.isBlank()) "Rate" else rateText,
              color = if (mode == QuickInputMode.RATE) BillSuperColors.PrimaryBlue else BillSuperColors.TextSecondary,
              style = AppTypography.bodyBold
            )
          }
        }
        IconButton(
          onClick = onGoReport,
          modifier = Modifier.background(BillSuperColors.Card2, RoundedCornerShape(10.dp)).padding(4.dp)
        ) {
          Icon(Icons.AutoMirrored.Outlined.ReceiptLong, contentDescription = "Receipt", tint = BillSuperColors.PrimaryBlue)
        }
        IconButton(
          onClick = {
            if (lines.isEmpty()) {
            ...
            val billRow = com.billsuper.data.db.BillWithItemsRow(
                billId = 0,
            ...
            val items = lines.map {
                com.billsuper.data.db.BillItemEntity(
                    billId = 0,
                    productId = null,
          ...
          modifier = Modifier.background(BillSuperColors.PrimaryBlue.copy(alpha = 0.1f), RoundedCornerShape(10.dp)).padding(4.dp)
        ) {
          Icon(Icons.Outlined.Print, contentDescription = "Print", tint = BillSuperColors.PrimaryBlue)
        }
      }

      if (!printMessage.isNullOrBlank()) {
          Text(printMessage!!, color = BillSuperColors.PrimaryBlue, style = AppTypography.bodySmall, modifier = Modifier.padding(horizontal = 12.dp))
      }
