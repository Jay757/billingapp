  ...
  ScreenSurface {
    Box(modifier = Modifier.fillMaxSize()) {
      ...
      if (!printMessage.isNullOrBlank()) {
        Text(printMessage!!, color = BillSuperColors.TextSecondary)
      }
    }
...
private fun BillCard(
  ...
  val paymentColor = when (row.paymentMethod.uppercase(Locale.getDefault())) {
    "CASH" -> BillSuperColors.PrimaryBlue
    "ONLINE" -> BillSuperColors.SuccessGreen
    "UPI" -> BillSuperColors.SuccessGreen
    "UP" -> BillSuperColors.SuccessGreen
    "CREDIT" -> BillSuperColors.AlertOrange
    else -> BillSuperColors.TextSecondary
  }
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(AppSpacing.md)) {
          ...
          Box(
            modifier = Modifier
              .size(44.dp)
              .clip(CircleShape)
              .background(BillSuperColors.Red.copy(alpha = 0.1f))
              .clickable(onClick = onDelete),
            contentAlignment = Alignment.Center
          ) {
            Icon(Icons.Outlined.Delete, contentDescription = "Delete", tint = BillSuperColors.Red, modifier = Modifier.size(20.dp))
          }
        }
      }
