    ...
    Box(modifier = Modifier.fillMaxSize()) {
      ...
      DarkCard(modifier = Modifier.fillMaxWidth()) {
        Column(
          ...
          Surface(
            color = if (state.status == BtConnectionState.Status.CONNECTED) AsliColors.SuccessGreen.copy(alpha = 0.1f) else AsliColors.PrimaryBlue.copy(alpha = 0.1f),
            ...
            Row(
              ...
              val iconColor = if (state.status == BtConnectionState.Status.CONNECTED) AsliColors.SuccessGreen else MaterialTheme.colorScheme.primary
...
private fun DeviceRow(
  ...
  DarkCard(modifier = Modifier.fillMaxWidth().clickable { onConnect() }) {
    Row(
      ...
      Surface(
        color = if (connected) AsliColors.SuccessGreen.copy(alpha = 0.1f) else MaterialTheme.colorScheme.surfaceVariant,
        shape = RoundedCornerShape(6.dp)
      ) {
        Text(
          if (connected) "CONNECTED" else "CONNECT",
          color = if (connected) AsliColors.SuccessGreen else MaterialTheme.colorScheme.onSurfaceVariant,
          style = MaterialTheme.typography.labelSmall.copy(fontWeight = FontWeight.Bold),
